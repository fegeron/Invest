...
Функция ОткрытьСчетПесочницы() Экспорт 
	
	СервисAPI = Справочники.СервисыAPI.TinkoffSandbox;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffOpenSandboxAccount;
...
Функция ЗапроситьСчетаПесочницы() Экспорт 
	
	СервисAPI = Справочники.СервисыAPI.TinkoffSandbox;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffGetSandboxAccounts;
...
Функция ЗапроситьСчета() Экспорт 
	
	СервисAPI = Справочники.СервисыAPI.Tinkoff;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffGetAccounts;
...
Функция ЗагрузитьИсторическиеКотировки(Instrument_Id, ПериодС, ПериодПо, ИнтервалСвечи) Экспорт 
	
	СервисAPI = Справочники.СервисыAPI.Tinkoff;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffGetCandles;
...
Функция ЗапроситьПозицииПоСчету(Account_id) Экспорт 
	
	СервисAPI = Справочники.СервисыAPI.Tinkoff;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffGetPositions;
	
	Данные = Новый Структура("Account_id", Account_id);	
	СтрокаJSON = РаботаСJSON.ЗаписатьДанныеВJSON(Данные);		
...
Функция ЗапроситьМаржинальныеПоказателиСчёта(Account_id) Экспорт 
	
	СервисAPI = Справочники.СервисыAPI.Tinkoff;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffGetMarginAttributes;
...
Функция ЗапроситьРасписанияТоргов(Биржа, ПериодС, ПериодПо) Экспорт
	СервисAPI = Справочники.СервисыAPI.Tinkoff;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffGetMarginAttributes;
	
	Данные = Новый Структура("Exchange, From, To", Биржа, ПериодС, ПериодПо);	
	СтрокаJSON = РаботаСJSON.ЗаписатьДанныеВJSON(Данные);		
...
Функция ЗапроситьВалюты(Instrument_status) Экспорт
	СервисAPI = Справочники.СервисыAPI.Tinkoff;
	
	МетодЗапроса = Справочники.МетодыЗапросов.TinkoffGetCurrencies;
	
	Данные = Новый Структура("Instrument_status", Instrument_status);	
	СтрокаJSON = РаботаСJSON.ЗаписатьДанныеВJSON(Данные);		
...
//	КонецЕсли;
	...
	Если СтруктураВозврата.ЕстьОшибки = Истина Тогда 
		РаботаСЖурналомОбмена.ЗаписатьОшибкуВЖурналОбмена(СтруктураОтбораОбмена, СтруктураВозврата.ТекстОшибки, СтруктураВозврата.КодСостояния);
	Иначе		
		СтруктураВозврата.Данные = СтрокаJSONОтвет;
		РаботаСЖурналомОбмена.ЗаписатьВыполненоВЖурналОбмена(СтруктураОтбораОбмена, СтрокаJSONОтвет, СтруктураВозврата.КодСостояния);
	КонецЕсли;
	//////////////////////////////////////////////////////		

...
Функция СоздатьHTTPЗапрос(АдресРесурса, СервисHttp, Логин = Неопределено, Пароль = Неопределено, ContentType = Неопределено, ContentLength = Неопределено) Экспорт
	...
	ТокенСсылка = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СервисHttp, "Токен");
	Токен = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ТокенСсылка, "Токен");
...
Процедура ОтправитьПодготовленныеДанныеВоВнешнююСистему() Экспорт 
	...
	Для Каждого ТекДанные Из ДанныеДляОтправки Цикл
		...
		СтруктураОтбораОбмена = РаботаСЖурналомОбмена.СтруктураИзмеренийОбмена(ТекДанные.Период, ТекДанные.МетодЗапроса, ТекДанные.ИдентификаторЗапроса, ТекДанные.СервисAPI);
		 
		ОтправитьДанныеВоВнешнююСистемуДляОбработки(СтруктураОтбораОбмена, ТекДанные.ТекстСообщения)
...
Функция ОтправитьДанныеВоВнешнююСистемуДляОбработки(Знач СтруктураОтбораОбмена, СтрокаJSON) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтрокаJSON) Тогда 	
		ЗафиксироватьОшибкуВСтруктуреВозврата(СтруктураВозврата, РаботаСHTTP.ИсключениеНетДанныхДляОтправки());
		Возврат СтруктураВозврата;
	КонецЕсли;	
	...
	СтрокаJSONОтвет = "";
		...
		Если МетодЗапроса.ИсходящийЗапрос Тогда 
			...
			// соединение			
			АдресРесурса = МетодЗапроса.Адрес;
